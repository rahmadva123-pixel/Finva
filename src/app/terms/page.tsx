            ...
            try {
                ...
                const docSnap = await getDoc(docRef);
                 if (docSnap.exists() && docSnap.data().terms) {
                    const data = docSnap.data().terms;
                    // Ensure sections is an array and has a type
                     if (Array.isArray(data.sections)) {
                        data.sections = data.sections.map((s: any) => ({ ...s, type: s.type || 'markdown' }));
                    } else {
                        data.sections = [{id: '1', title: 'General Terms', type: 'markdown', description: data.content || 'The terms and conditions will be listed here.'}];
                    }
                    setPageData(data);
                } else {
                    setPageData({
                        title: "Terms & Conditions",
                        sections: [{id: '1', title: 'General Terms', type: 'markdown', description: 'The terms and conditions will be listed here.'}],
                    });
                }
            } catch (error) {
