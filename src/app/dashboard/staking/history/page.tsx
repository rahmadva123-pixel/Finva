            ...
            try {
                ...
                const historyData = querySnapshot.docs.map(doc => {
                    ...
                    return {
                        id: doc.id,
                        ...data,
                        returnAmount: data.amount + (data.amount * data.returnPercentage / 100)
                    } as StakingHistory;
        ...
        <DashboardLayout>
            <div className="space-y-6">
                <div>
                    ...
                    <p className="text-muted-foreground">A record of all your completed stakes.</p>
                </div>
                <Card>
                    <CardHeader>
                        <CardTitle>Completed Stakes</CardTitle>
                        <CardDescription>Review the details of your past staking activities.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        {loading ? (
                            <div className="flex justify-center p-8"><Loader2 className="h-8 w-8 animate-spin" /></div>
                        ) : history.length === 0 ? (
                            <p className="text-center text-muted-foreground p-8">You have no completed stakes.</p>
                        ) : (
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                ...
                                <TableBody>
                                    {history.map((item) => (
                                        <TableRow key={item.id}>
                                            ...
                                            <TableCell>{format(new Date(item.startDate.seconds * 1000), "PPp")}</TableCell>
                                            <TableCell>{format(new Date(item.endDate.seconds * 1000), "PPp")}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
